...
// Geolocate, get lattitude and longitude
...
// Convert longitude and lattitude to address (Reverse-Geocoding)

function reverse_geolocate(API){
    return new Promise ((resolve,reject) =>{
        request.get(API, function callback(err,response,body){
            if (err){
                console.log("Post Failed.Error: ",err)
            }
            body = JSON.parse(body)
...
function getRoute(API){
    fetch(API,{mode:'no-cors'}).then(response=>{
        ...
        fs.writeFile("route.html",data , function(err){
            if (err){
                console.log(err)
            }
...
function getRoute(API){
    window.fetch(API,{mode:'no-cors'}).then(response=>{
        return response.text()    
    }) .then (data=>{
        window.fs.writeFile("route.html",data , function(err){
            if (err){
                console.log(err)
            }
            document.getElementById("content").innerHTML='<object type="type/html" data="route.html" ></object>';
            console.log("File saved")
        })
    }) .catch(err =>{
        console.log(err)
